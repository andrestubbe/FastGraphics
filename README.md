...
`java.awt.Graphics2D` is convenient but slow. Its immediate-mode API creates CPU bottlenecks, and Java2D's software
rasterizer limits performance to ~100-200 simple shapes per frame.
...
- **AntiAliasing**: RenderingHints.KEY_ANTIALIASING is supported via API, but DirectX 11 MSAA requires Swap Chain
  configuration (not runtime-switchable)
...
|---------------------|-------------------|--------------------|
| Windows 10/11 (x64) | ✅ Fully Supported | DirectX 11 native  |
| Linux               | 🚧 Planned        | OpenGL/Vulkan port |
| macOS               | 🚧 Planned        | Metal port         |
